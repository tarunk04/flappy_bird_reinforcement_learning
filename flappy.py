...
def mainGame(movementInfo):
    ...
    while True:
        ...
        distance = upperPipes[currPipe]['x'] - playerx - IMAGES['player'][0].get_width()

        #updating the current target pipe to the next pipe
        if distance + IMAGES['pipe'][0].get_width() + IMAGES['player'][0].get_width() <  0:
            currPipe += 1
            distance = upperPipes[currPipe]['x'] - playerx - IMAGES['player'][0].get_width()

        #distance of upper pipe from the upper part of bird
        upper_pipe_dis = (playery - pad ) - (lowerPipes[currPipe]['y'] - PIPEGAPSIZE)

        #distance of lower pipe from the lower part of bird
        lower_pipe_dis = lowerPipes[currPipe]['y'] - (playery + IMAGES['player'][0].get_width() + pad)
        ...
        #input numpy array to NN Model
        input = np.array([[ upper_pipe_dis  , lower_pipe_dis   , distance ]])/500
        act = np.argmax(model.predict(input)[0])

        #if predicted action is 1 the flapping the bird
        if act == 1:
            if playery > -2 * IMAGES['player'][0].get_height():
                playerVelY = playerFlapAcc
                playerFlapped = True
...
def checkCrash(player, upperPipes, lowerPipes):
    ...
    if player['y'] + player['h'] >= BASEY - 1:
        ...
        for uPipe, lPipe in zip(upperPipes, lowerPipes):
            ...
            #print(" hitp: ", pHitMask, "hitu: ", uHitmask,"  hitl:  ",lHitmask)
